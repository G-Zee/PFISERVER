    ...
    class collectionFilter {
        constructor(collection, filterParams) {
            ...
            if (isNaN(this.limit) || isNaN(this.offset)) {
                this.limit = 0;     //Limit and offset are used to only send 5 elements after each load
                this.offset = 0;
            }
        }
        ...
        get() {
            ...
            if (this.sortFields.length > 0)
                this.sort();
            if (this.limit != 0) {
                if(this.limit + this.offset <= this.filteredCollection.length) 
                    res = this.filteredCollection.slice(this.offset, this.limit + this.offset)
                else
                    res = this.filteredCollection.slice(this.offset, this.filteredCollection.length);
            return res;
            }
